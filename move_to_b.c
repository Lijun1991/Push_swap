...
static void	test_rotate(int nbr, t_pinfo *info, t_plst *lstb, int count)
{
	int tmp;
...
int		get_right_nbr(t_plst *lst, t_plst *lstb, t_pinfo *info)
{
	...
	while (cur)
	{
		...
		test_rotate(cur->data, info, lstb, count);
	...
	while (cur1)
	{
		...
		test_rotate(cur1->data, info, lstb, count);
